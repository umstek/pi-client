...
const CountdownView = Cycle.component('CountdownView', (interactions, props) => {
  ...
  const finalStream = clockStream.withLatestFrom(pauseStream, restartStream,
    (clock, pause, restart) => ({
      restart, // whether counter needed to be restarted
      pause, // Need to transfer the state to internal function
  ...
  return finalStream.map(
    (state) => {
      ...
      return (
        <div>
          <Card
            title="Pomodoro Timer" bordered={false} extra={
              <ButtonGroup>
                ...
                <Tooltip title="Restart">
                  <Button
                    icon="reload" disabled={countdown <= 0}
          ...
          >
            <Row gutter={8} type="flex" justify="space-around">
              <Col span={8}>
                <Progress
                  width={256} strokeWidth={2} type="circle"
                  status={status}
